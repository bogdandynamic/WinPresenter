...
namespace Examples.ExamplesConsole
{
    ...
    public partial class NetManager
    {
        ...
        public void newMessage(string passMessage, bool peerSend, int peerExitCode)
        {
            ...
            Console.WriteLine("newMessage executes");

            peerMessage = passMessage;
            sendMessage = peerSend;
            exitCode = peerExitCode;

            Console.WriteLine("newMessage finishes");
        ...
        private void HandleIncomingMessagePacket(PacketHeader header, Connection connection, string incomingString)
        {
            NetManager propTest = new NetManager();
            Console.WriteLine("\n  ... Incoming message from " + connection.ToString() + " saying '" + incomingString + "'.");


            if (incomingString == "addressDecoded")
            {

                string passMessage = "fetch inv.xml";
                bool peerSend = true;
                int peerExitCode = 0;
            ...
            else if (incomingString == "openFetchListener")
            {
                //NetManager propTest = new NetManager();
                fetchListener = true;

                string passMessage = "fetchListenerOpened";
                bool peerSend = true;
                int peerExitCode = 0;
            ...
            else if (incomingString == "closeFetchListener")
            {
                //NetManager propTest = new NetManager();
                fetchListener = false;

                string passMessage = "fetchListenerClosed";
                bool peerSend = true;
                int peerExitCode = 0;
